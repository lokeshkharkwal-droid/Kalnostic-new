-- ─────────────────────────────────────────────────────────────────────────────
-- Row-Level Security policies + partial unique indexes (CLAUDE.md §4.3).
--
-- This migration is a snapshot of prisma/rls.sql so that `prisma migrate deploy`
-- fully provisions a fresh database (schema in 0_init, then RLS + the partial
-- unique indexes Prisma cannot express in schema.prisma). It runs AFTER 0_init,
-- so every table it references already exists.
--
-- SOURCE OF TRUTH: prisma/rls.sql. During local `prisma db push` development
-- that file is (re)applied idempotently. When RLS changes, update prisma/rls.sql
-- AND add a NEW migration that re-runs the full, updated rls.sql (it is written
-- to be idempotent — DROP ... IF EXISTS / CREATE ... IF NOT EXISTS / FORCE — so
-- re-running it is safe). Do not edit this file after it has been applied.
-- ─────────────────────────────────────────────────────────────────────────────
