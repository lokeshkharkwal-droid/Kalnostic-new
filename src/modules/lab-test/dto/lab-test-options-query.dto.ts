...
/**
 * Query DTO for the lightweight `GET /lab-tests/options` endpoint (id + name
 * only), used by the searchable, paginated selector (e.g. the Add Outsource
 * Center form). All fields are optional:
 *
 * - `branchId` — include only active lab tests on this branch.
 * - `search` — case-insensitive match against the lab test `testName`.
 ...
 */
export class LabTestOptionsQueryDto extends PaginationQueryDto {
  @IsOptional()
  @IsUUID()
  branchId?: string;
