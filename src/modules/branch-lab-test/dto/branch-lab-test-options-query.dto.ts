...
/**
 * Query DTO for the lightweight `GET /branch-lab-tests/options` endpoint (id +
 ...
 *
 * - `search` — case-insensitive match against the branch lab test `testName`.
 ...
 *   `isPreferenceTest: true`. Opt-in and defaults to false so every existing
 *   caller keeps seeing the full catalogue unchanged; only the Create-Order
 *   picker's unsearched view should pass this.
 */
export class BranchLabTestOptionsQueryDto extends PaginationQueryDto {
  @IsOptional()
  @IsString()
  @MaxLength(255)
  search?: string;
  ...
  /** Narrow the unsearched view to preference tests only (Create-Order picker). */
  @IsOptional()
  @ToBoolean()
  @IsBoolean()
