 ...
 * import-picker's "Available to Add" listing (`GET /master-data/import/lab-panels`)
 * — no client-supplied id list. The backend re-resolves every Master Data lab
 * panel matching these filters (excluding ones already in the target list) and
 * imports all of them, so a filtered "select all" never round-trips a
 * potentially huge id array through the client. Panels have no sub-category
 * (unlike lab tests).
 */
export class ImportBranchLabPanelsByFilterDto {
